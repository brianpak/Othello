...
void binaryULL(ull n) {
  ull i;
  // Reverse loop
  for (i = 1ULL << 63; i > 0; i >>= 1)
      printf("%u", !!(n & i));
  printf("\n");
}
...
int main (int argc, char * argv[])
{
  ...
  while ((opt = getopt(argc, argv, "t:d:")) != -1) {
    ...
    case 't':
       ...
       break;
    default: /* '?' */
       fprintf(stderr, "Usage: -t [number of threads] -d [number of depths] \n");
       exit(-1);
    }
  }

  const char* t = THREAD;
  if (0!= __cilkrts_set_param("nworkers",t))
  {
